...
export default function Sidebar() {
  ...
  const menuItems = [
    { name: "Inicio (Extracao)", href: "/", icon: Home },
  ...
  return (
    <div className="w-64 bg-white border-r border-gray-200 h-screen fixed left-0 top-0 flex flex-col justify-between z-40 transition-colors duration-300">
      <div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href}>
                <div
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium text-sm mb-1 ${
                    isActive
            ...
            >
              ...
              <ChevronDown
                ...
                className={`transition-transform duration-300 ${
                  pathname.startsWith("/cadastros") ? "text-blue-600" : "text-gray-400"
                } ${isCadastrosOpen ? "rotate-180" : ""}`}
              />
            </button>

            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isCadastrosOpen ? "max-h-64 opacity-100 mt-1" : "max-h-0 opacity-0"}`}>
              <div className="ml-9 flex flex-col gap-1 border-l-2 border-gray-100 pl-3 py-1">
                ...
                <Link href="/cadastros/cidades">
                  <div className={`p-2 text-sm rounded-lg transition-colors font-medium ${
                    pathname === "/cadastros/cidades" ? "text-blue-600 bg-blue-50" : "text-gray-500 hover:text-blue-600 hover:bg-gray-50"
                  }`}>
                    Cidades (Rotas)
                  </div>
                </Link>
      ...
      <div className="p-4 border-t border-gray-100 space-y-2">
        <button
          onClick={toggleTheme}
          className="w-full flex items-center justify-between px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 transition-all duration-300 font-medium text-sm group"
        ...
        >
          <div className="flex items-center gap-3">
            {theme === "dark" ? (
              ...
              <Moon className="w-5 h-5 text-gray-400 transition-transform duration-500 group-hover:-rotate-12" />
            )}
            <span>{theme === "dark" ? "Modo Claro" : "Modo Escuro"}</span>
          </div>

          <div className={`relative w-10 h-5 rounded-full transition-colors duration-300 ${theme === "dark" ? "bg-blue-600" : "bg-gray-300"}`}>
