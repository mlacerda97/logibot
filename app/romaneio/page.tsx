...
export default function RomaneioPage() {
  ...
  const criarViagem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Amarra as notas fiscais à viagem criada (Usando RPC para evitar erro de PATCH/CORS)
      const { error: erroEntregas } = await supabase.rpc('roteirizar_notas', {
        notas_ids: entregasSelecionadas, // O array de IDs que você já tem
        id_viagem: novaViagem.id        // O ID da viagem que acabou de ser criada
      });
