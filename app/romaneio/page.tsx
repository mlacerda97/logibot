...
const parseIsoDate = (valor: string | null | undefined): Date | null => {
...
const formatarData = (valor: string | null | undefined): string => {
  const data = parseIsoDate(valor);
  return data ? data.toLocaleDateString("pt-BR") : "--";
};

const statusPrazoEntrega = (dataPrevista: string | null | undefined): { label: string; classes: string } => {
  const data = parseIsoDate(dataPrevista);
  if (!data) {
    return {
      label: "Sem previsão",
  ...
  const inicioPrev = new Date(data.getFullYear(), data.getMonth(), data.getDate());
  const diasRestantes = Math.floor((inicioPrev.getTime() - inicioHoje.getTime()) / (1000 * 60 * 60 * 24));

  if (diasRestantes <= 0) {
    return {
      label: diasRestantes < 0 ? "Atrasada" : "Vence hoje",
  ...
  if (diasRestantes <= 2) {
    return {
      label: `${diasRestantes}d para vencer`,
...
export default function RomaneioPage() {
  ...
  const pendentesClassificados = useMemo(() => {
    return todosPendentes.map((entrega) => {
      ...
      const municipioRef = entrega.municipio_entrega || cidade;
      const ufRef = entrega.uf_entrega || uf;
      const chave = `${normalizarTexto(municipioRef)}|${normalizarTexto(ufRef)}`;
      ...
      return {
        ...
        municipioReferencia: municipioRef || "",
        ufReferencia: ufRef || "",
        dataEmissaoCte: entrega.data_emissao_cte || null,
        dataPrevistaEntrega: entrega.data_prevista_entrega || null
  ...
  const pendentesFiltrados = useMemo(() => {
    ...
    const inicioEmissao = parseIsoDate(filtroEmissaoDe);
    const fimEmissao = parseIsoDate(filtroEmissaoAte);

    if (inicioEmissao) inicioEmissao.setHours(0, 0, 0, 0);
    if (fimEmissao) fimEmissao.setHours(23, 59, 59, 999);

    const filtrados = pendentesClassificados.filter((ent) => {
      const bateBusca =
        !termo ||
        String(ent.cte_origem || "").toLowerCase().includes(termo) ||
        String(ent.numero_nf || "").toLowerCase().includes(termo) ||
        String(ent.cliente_nome || "").toLowerCase().includes(termo) ||
        String(ent.municipioReferencia || "").toLowerCase().includes(termo);
      const bateRota = !filtroRota || ent.rota === filtroRota;
      const bateCor = !filtroCor || ent.cor === filtroCor;
      const emissao = parseIsoDate(ent.dataEmissaoCte);
      const bateEmissaoDe = !inicioEmissao || (emissao ? emissao >= inicioEmissao : false);
      const bateEmissaoAte = !fimEmissao || (emissao ? emissao <= fimEmissao : false);
      return bateBusca && bateRota && bateCor && bateEmissaoDe && bateEmissaoAte;
    });

    return filtrados.sort((a, b) => {
      const emissaoA = parseIsoDate(a.dataEmissaoCte)?.getTime() ?? Number.MAX_SAFE_INTEGER;
      const emissaoB = parseIsoDate(b.dataEmissaoCte)?.getTime() ?? Number.MAX_SAFE_INTEGER;
      if (emissaoA !== emissaoB) return emissaoA - emissaoB;

      const createdA = parseIsoDate(a.created_at)?.getTime() ?? Number.MAX_SAFE_INTEGER;
      const createdB = parseIsoDate(b.created_at)?.getTime() ?? Number.MAX_SAFE_INTEGER;
      return createdA - createdB;
    });
  }, [pendentesClassificados, buscaLista, filtroRota, filtroCor, filtroEmissaoDe, filtroEmissaoAte]);
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-8 focus:outline-none" onPaste={handlePaste} tabIndex={0}>
      <div className="max-w-6xl mx-auto">
        ...
        <form id="form-romaneio" onSubmit={criarViagem} className="space-y-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Carga Selecionada</p>
              <p className="text-gray-900 font-extrabold text-lg">
                {totalItensSelecionados} <span className="text-gray-500 text-base font-semibold">Notas / Destinos</span>
              </p>
            </div>
            <button
              type="submit"
              disabled={salvando || totalItensSelecionados === 0}
              className="bg-blue-600 hover:bg-blue-500 disabled:bg-gray-300 text-white font-black py-3 px-8 rounded-2xl shadow-md flex items-center justify-center gap-2 transition-all active:scale-95"
            >
              {salvando ? "Enviando para IA..." : <><span>Criar Romaneio (IA)</span><Truck size={18} /></>}
            </button>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-[1.45fr_1fr] gap-8">
            <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 flex flex-col h-[620px] overflow-hidden">
                ...
                {modoEntrada === "lista" && (
                  <div className="flex flex-col h-full">
                    <div className="relative mb-4">
                      ...
                      <input type="text" placeholder="Filtrar por CT-e, NF, cliente ou municipio..." value={buscaLista} onChange={(e) => setBuscaLista(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <input
                        ...
                        className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-semibold text-gray-700 outline-none focus:ring-2 focus:ring-blue-500"
                        title="Emissao a partir de"
                      />
                      <input
                        type="date"
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-2">
                      {carregandoLista ? <div className="text-center py-10"><Loader2 className="animate-spin inline text-blue-500" /><p className="text-xs text-gray-400 font-bold mt-2">Carregando estoque...</p></div> :
                        pendentesFiltrados.map((ent) => {
                          const prazo = statusPrazoEntrega(ent.dataPrevistaEntrega);
                          return (
                            <div key={ent.id} onClick={() => toggleEntrega(ent)} className={`p-3 rounded-xl border-2 transition-all cursor-pointer ${entregasSelecionadas.includes(ent.id) ? "border-blue-600 bg-blue-50 shadow-sm" : "border-gray-100 hover:border-blue-200 bg-white"}`}>
                              ...
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                <span className="text-[9px] font-black text-gray-500 bg-gray-100 border border-gray-200 rounded px-1.5 py-0.5 uppercase">
                                  Entrega: {ent.municipioReferencia || "--"}/{ent.ufReferencia || "--"}
                                </span>
                                <span className={`text-[9px] font-black border rounded px-1.5 py-0.5 uppercase inline-flex items-center gap-1 ${prazo.classes}`}>
                                  <span className="w-1.5 h-1.5 rounded-full bg-current" />
                                  {prazo.label}
                                </span>
                              </div>
                              <div className="flex items-center gap-2 mt-1">
                                <span className="text-[9px] font-semibold text-gray-500">Emissao: {formatarData(ent.dataEmissaoCte)}</span>
                                <span className="text-[9px] font-semibold text-gray-500">Prev: {formatarData(ent.dataPrevistaEntrega)}</span>
                              </div>
            ...
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex flex-col h-[620px]">
              <div className="grid grid-cols-1 gap-4 pb-5 border-b border-gray-100">
              ...
              <div className="flex flex-col flex-1 pt-5 min-h-0">
